...
public class IndexingJob extends NutchTool implements Tool {
  ...
  public static class IndexerMapper extends
    ...
    public void printMap(  java.util.Map<java.lang.CharSequence, java.lang.CharSequence> in ) {
        for (java.lang.CharSequence  key : in.keySet()) {
            java.lang.CharSequence value = in.get(key);
    ...
    public void map(String key, WebPage page, Context context)
      ...
      LOG.info("IndexerMapper map key:"+key+" ");
      System.out.println(context.getCurrentValue().getClass().getName());
      page = context.getCurrentValue();
      //ParseUtil.process(key, page);
      (new ParseUtil(context.getConfiguration())).process(key, page);
      ParseStatus pstatus = page.getParseStatus();
       System.out.println(">>> "+page);
      //LOG.info("pstatus " +pstatus.toString()+ " pstatus.getMinorCode() "+pstatus.getMinorCode());
      if (pstatus == null || !ParseStatusUtils.isSuccess(pstatus)
          || pstatus.getMinorCode() == ParseStatusCodes.SUCCESS_REDIRECT) {
        return; // filter urls not parsed
      }
      Utf8 mark = Mark.UPDATEDB_MARK.checkMark(page);
      printMap( page.getMarkers() );
      LOG.info("Mark.... "+page.getMarkers().toString());
      if (mark == null) {
      ...
      LOG.info("NutchDocument:"+doc.toString());
  ...
  public static <K, V extends Persistent> DataStore<K, V> createWebStore(
    ...
    System.out.println("-----------"+schemaPrefix+"---"+schema+"-------------");
  ...
  public static String[] toStringArray(Collection<WebPage.Field> fields) {
    ...
    for (int i = 0; i < arr.length; i++) {
      ...
      System.out.println("initMapperJob> query.setFields>"+arr[i]);
    }
    return arr;
  }
  private void fix(Job job, MapFieldValueFilter<String, WebPage> filter, Collection<WebPage.Field> fields) {
  try{
    ...
    GoraMapper.initMapperJob(job, query, store, String.class, NutchDocument.class,
        IndexerMapper.class,  true);
  ...
  public void index(String batchId) throws Exception {
    LOG.info("IndexingJob: starting "+batchId);
    ...
    LOG.info("IndexingJob: commit "+getConf().getBoolean(SolrConstants.COMMIT_INDEX, true));
