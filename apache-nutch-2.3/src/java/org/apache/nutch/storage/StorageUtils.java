...
public class StorageUtils {
  ...
  public static void test(DataStore<String, WebPage> store, Query<String, WebPage> query ){
    try {
    ...
    while (result.next()) {
      try {
        // only count keys in the store for the current id
          System.out.println("test key "+result.getKey().toString());
          System.out.println("test value "+result.get().toString());
          count++;
      } catch (Exception e) {
        e.printStackTrace();
      }
